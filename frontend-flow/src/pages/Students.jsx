...
const CLASSES = ["Form 1A","Form 1B","Form 1C","Form 2A","Form 2B","Form 2C","Form 3A","Form 3B","Form 3C","Form 4A","Form 4B","Form 4C"];
const FEE_STRUCTURES = { "Form 1A":22000,"Form 1B":22000,"Form 1C":22000,"Form 2A":24000,"Form 2B":24000,"Form 2C":24000,"Form 3A":26000,"Form 3B":26000,"Form 3C":26000,"Form 4A":28000,"Form 4B":28000,"Form 4C":28000 };
...
function Avatar({ name, size = 32 }) {
  ...
  return (
    <div style={{ width: size, height: size, borderRadius: 8, background: `hsl(${hue},55%,28%)`, border: `1px solid hsl(${hue},55%,38%)`, display: "flex", alignItems: "center", justifyContent: "center", fontSize: size * 0.35, fontWeight: 600, color: `hsl(${hue},80%,78%)`, flexShrink: 0, letterSpacing: 0.5 }}>
      {initials}
    </div>
  );
}
  ...
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
      <div style={{ flex: 1, height: 4, background: "var(--surface3)", borderRadius: 2, overflow: "hidden" }}>
        <div style={{ width: `${pct}%`, height: "100%", background: pct >= 100 ? "var(--accent)" : pct > 0 ? "var(--accent3)" : "var(--danger)", borderRadius: 2, transition: "width .3s" }} />
      </div>
...
function AddStudentModal({ onClose, onAdd }) {
  const { token, plan, canUse } = useAuth();
  ...
  const handleSubmit = async () => {
    ...
    try {
      const res = await axios.post(`${API}/api/students`, { name: form.name.trim(), adm: form.adm.trim(), cls: form.cls, fee: feeNum, paid: paidNum, phone: form.phone.trim() }, { headers: { Authorization: `Bearer ${token}` } });
  ...
  return (
    <>
      ...
      <div className="modal-box" style={{ maxHeight: "92vh", display: "flex", flexDirection: "column", overflowY: "auto" }}>
        ...
        <div style={{ display: "flex", borderBottom: "1px solid var(--border)", padding: "0 22px", flexShrink: 0 }}>
          {["Student info", "Review & save"].map((label, i) => (
            <div key={i} onClick={() => i + 1 < step && setStep(i + 1)} style={{ padding: "10px 0", marginRight: 20, fontSize: 12, fontWeight: 500, color: step === i + 1 ? "var(--accent)" : "var(--text3)", borderBottom: `2px solid ${step === i + 1 ? "var(--accent)" : "transparent"}`, cursor: i + 1 < step ? "pointer" : "default", transition: "all .15s" }}>
              {label}
            </div>
          ))}
        </div>
          ...
          {step === 1 && (
            <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
              <div className="field-group">
                ...
                <input style={inp} value={form.name} onChange={set("name")} placeholder="e.g. Amina Wanjiru" />
              ...
              <div className="field-group">
                ...
                <input style={inp} value={form.adm} onChange={set("adm")} placeholder="e.g. ADM/2025/001" />
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
                ...
                <div className="field-group">
                  ...
                  <input style={inp} value={form.phone} onChange={set("phone")} placeholder="07XX XXX XXX" />
                </div>
              </div>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
                <div className="field-group">
                  <label className="settings-label">Term fee (KES) *</label>
                  <input style={inp} type="number" value={form.fee} onChange={set("fee")} placeholder="e.g. 24000" />
                  {errors.fee && <div style={{ fontSize: 11.5, color: "var(--danger)", marginTop: 4 }}>{errors.fee}</div>}
                  {form.cls && FEE_STRUCTURES[form.cls] && <div style={{ fontSize: 11, color: "var(--text3)", marginTop: 4 }}>Auto-filled from {form.cls} structure</div>}
                </div>
                <div className="field-group">
                  <label className="settings-label">Amount paid (KES)</label>
                  <input style={inp} type="number" value={form.paid} onChange={set("paid")} placeholder="0 if unpaid" />
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
              <div style={{ background: "var(--surface2)", border: "1px solid var(--border)", borderRadius: 10, padding: "14px 16px", display: "flex", alignItems: "center", gap: 14 }}>
                ...
                <div style={{ flex: 1 }}>
                  ...
                  <div style={{ fontSize: 12, color: "var(--text3)", marginTop: 2 }}>{form.cls}{form.adm && ` · ${form.adm}`}{form.phone && ` · ${form.phone}`}</div>
                </div>
                <Badge status={status} />
              </div>
              <div style={{ background: "var(--surface2)", border: "1px solid var(--border)", borderRadius: 10, overflow: "hidden" }}>
                ...
                <div style={{ padding: "10px 14px", borderTop: "1px solid var(--border)" }}><ProgressBar fee={feeNum} paid={paidNum} /></div>
              </div>
...
export default function StudentsPage() {
  ...
  const filtered = useMemo(() => students.filter(s => {
    ...
    return (search === "" || s.name.toLowerCase().includes(search.toLowerCase()) || s.cls.toLowerCase().includes(search.toLowerCase()) || (s.adm || "").toLowerCase().includes(search.toLowerCase()))
      && (filterStatus === "all" || st === filterStatus)
      && (filterClass  === "all" || s.cls === filterClass);
  }), [students, search, filterStatus, filterClass]);

  const stats = useMemo(() => ({ total: students.length, paid: students.filter(s => getStatus(s.fee, s.paid) === "paid").length, partial: students.filter(s => getStatus(s.fee, s.paid) === "partial").length, overdue: students.filter(s => getStatus(s.fee, s.paid) === "overdue").length }), [students]);
    ...
    <>
      <div className="topbar">
        <div>
          ...
          <div className="topbar-sub">{students.length} enrolled · Term 2, 2025</div>
        </div>
      ...
      <div className="page-content">
        ...
        <div style={{ display: "flex", gap: 12, marginBottom: 20, flexWrap: "wrap" }}>
          {[
            { label: "All students", value: stats.total,   key: "all",     color: "var(--text)" },
            { label: "Fully paid",   value: stats.paid,    key: "paid",    color: "var(--accent)" },
            ...
            <div key={s.key} onClick={() => setFilterStatus(s.key)} style={{ background: filterStatus === s.key ? "var(--surface2)" : "var(--surface)", border: `1px solid ${filterStatus === s.key ? "var(--surface3)" : "var(--border)"}`, borderTop: `${filterStatus === s.key ? "2px" : "1px"} solid ${filterStatus === s.key ? s.color : "var(--border)"}`, borderRadius: 10, padding: "10px 16px", cursor: "pointer", minWidth: 110 }}>
              <div style={{ fontSize: 20, fontFamily: "'DM Serif Display',serif", color: s.color, lineHeight: 1 }}>{s.value}</div>
              <div style={{ fontSize: 11, color: "var(--text3)", marginTop: 4, textTransform: "uppercase", letterSpacing: .8 }}>{s.label}</div>
            </div>
          ))}
        </div>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 16, flexWrap: "wrap" }}>
          <div style={{ position: "relative", flex: "1 1 260px" }}>
            <svg style={{ position: "absolute", left: 12, top: "50%", transform: "translateY(-50%)", pointerEvents: "none" }} width="15" height="15" fill="none" viewBox="0 0 24 24" stroke="var(--text3)" strokeWidth="2">
          ...
          <select value={filterClass} onChange={e => setFilterClass(e.target.value)} style={{ height: 40, background: "var(--surface)", border: "1px solid var(--border)", color: "var(--text)", borderRadius: 9, padding: "0 12px", fontSize: 13.5, width: "auto" }}>
            <option value="all">All classes</option>
            {CLASSES.map(c => <option key={c} value={c}>{c}</option>)}
          </select>
          <div style={{ marginLeft: "auto", fontSize: 12, color: "var(--text3)" }}>Showing {filtered.length} of {students.length}</div>
        </div>
        ...
        <div style={{ background: "var(--surface)", border: "1px solid var(--border)", borderRadius: 12, overflow: "hidden" }}>
          {loading ? (
            ...
            <table>
              <thead>
                <tr>
                  {["Student", "Adm No.", "Class", "Term Fee", "Paid", "Balance", "Progress", "Status"].map(h => <th key={h}>{h}</th>)}
                </tr>
              </thead>
              <tbody>
                {filtered.length === 0 ? (
                  <tr><td colSpan={8} style={{ padding: "40px 16px", textAlign: "center", color: "var(--text3)" }}>No students match your filters</td></tr>
                ) : filtered.map((s, i) => {
                  const status  = getStatus(s.fee, s.paid);
                  const balance = s.fee - s.paid;
                  return (
                    <tr key={s.id}>
                      <td><div style={{ display: "flex", alignItems: "center", gap: 10 }}><Avatar name={s.name} /><span style={{ fontWeight: 500, color: "var(--text)" }}>{s.name}</span></div></td>
                      <td>{s.adm || "—"}</td>
                      <td>{s.cls}</td>
                      ...
                      <td style={{ color: balance === 0 ? "var(--text3)" : "var(--accent3)", fontVariantNumeric: "tabular-nums", fontWeight: 500 }}>{balance === 0 ? "—" : balance.toLocaleString()}</td>
      ...
      {toast && (
        <div style={{ position: "fixed", bottom: 28, right: 28, zIndex: 100, background: "var(--surface)", border: "1px solid var(--border)", borderLeft: `3px solid ${toast.type === "error" ? "var(--danger)" : "var(--accent)"}`, borderRadius: 10, padding: "12px 18px", fontSize: 13, color: "var(--text)", boxShadow: "var(--shadow)", display: "flex", alignItems: "center", gap: 10 }}>
          <span style={{ color: toast.type === "error" ? "var(--danger)" : "var(--accent)" }}>{toast.type === "error" ? "✕" : "✓"}</span>
          {toast.msg}
        </div>
      )}
    </>
