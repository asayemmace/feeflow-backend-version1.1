  ...
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
      <div style={{ flex: 1, height, background: "var(--surface3)", borderRadius: 2, overflow: "hidden" }}>
        <div style={{
          width: `${pct}%`, height: "100%", borderRadius: 2, transition: "width .4s ease",
          background: pct >= 100 ? "var(--green)" : pct > 0 ? "var(--amber)" : "var(--red)",
        }} />
      </div>
...
function StudentProfileModal({ studentId, onClose, token }) {
  const [data,    setData]    = useState(null);
  const [loading, setLoading] = useState(true);
  const [error,   setError]   = useState("");

  useEffect(() => {
    axios.get(`${API}/api/students/${studentId}/payments`, {
    ...
    <>
      ...
      <div style={{
        ...
        <div style={{ padding: "18px 20px", borderBottom: "1px solid var(--border)", display: "flex", alignItems: "center", justifyContent: "space-between", flexShrink: 0 }}>
          ...
          <button onClick={onClose} style={{ width: 30, height: 30, borderRadius: 7, background: "var(--surface2)", border: "1px solid var(--border)", display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", color: "var(--text2)", fontSize: 14, transition: "all .15s" }}
            ...
            <div style={{ padding: "20px" }}>
              ...
              <div style={{ background: "var(--surface2)", border: "1px solid var(--border)", borderRadius: 12, padding: 16, marginBottom: 18 }}>
                ...
                <div style={{ fontSize: 11, color: "var(--text3)", marginTop: 5 }}>
                  ...
                  {data.student.daysOverdue > 0 && (
                    <span style={{ marginLeft: 10, color: "var(--red)" }}>· {data.student.daysOverdue} days overdue</span>
                  )}
                </div>
              </div>

              {/* Payment history by term */}
              <div style={{ fontSize: 12, fontWeight: 600, color: "var(--text3)", textTransform: "uppercase", letterSpacing: ".8px", marginBottom: 10 }}>
                Payment History
              </div>
                ...
                <div key={term.termId} style={{ marginBottom: 16 }}>
                  ...
                  <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 8 }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                      ...
                      {term.status === "active" && (
                        <span style={{ fontSize: 10, fontWeight: 700, padding: "2px 7px", borderRadius: 20, background: "var(--green-bg)", color: "var(--green)", border: "1px solid var(--green-border)" }}>ACTIVE</span>
                      )}
                    </div>
                  ...
                  <div style={{ background: "var(--surface2)", border: "1px solid var(--border)", borderRadius: 10, overflow: "hidden" }}>
                        ...
                        <div style={{ flex: 1, minWidth: 0 }}>
                          ...
                          <div style={{ fontSize: 11, color: "var(--text3)", marginTop: 1, display: "flex", alignItems: "center", gap: 6, flexWrap: "wrap" }}>
                            ...
                            {p.feeBreakdown?.length > 0 && p.feeBreakdown.map((fb, fi) => (
                              <span key={fi} style={{ fontSize: 10, padding: "1px 6px", borderRadius: 4, background: "var(--surface3)", border: "1px solid var(--border)", color: "var(--text3)" }}>
                                {fb.typeName}
                              </span>
                            ))}
                          </div>
                        </div>
                        <div style={{ fontSize: 14, fontWeight: 600, color: "var(--green)", fontVariantNumeric: "tabular-nums", flexShrink: 0 }}>
                    ...
                    <div style={{ display: "flex", justifyContent: "space-between", padding: "9px 16px", background: "rgba(0,0,0,0.15)", borderTop: "1px solid var(--border)" }}>
                      ...
                      <span style={{ fontSize: 13, fontWeight: 700, color: "var(--green)", fontVariantNumeric: "tabular-nums" }}>
                        KES {Number(term.paid).toLocaleString()}
                      </span>
                    </div>
                  </div>
                </div>
      ...
      {feeTypes.map(ft => {
        ...
        return (
          <div key={ft.id} style={{
            ...
            <div onClick={() => onToggle(ft.id)} style={{
            ...
            <span onClick={() => onToggle(ft.id)} style={{ flex: 1, fontSize: 13.5, color: isSelected ? "var(--text)" : "var(--text2)", fontWeight: isSelected ? 500 : 400, cursor: "pointer" }}>
              {ft.name}
            </span>
            {isSelected ? (
              <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                <span style={{ fontSize: 11.5, color: "var(--text3)" }}>KES</span>
                <input type="number" min="0" value={amount || ""} onChange={e => onFeeChange(ft.id, e.target.value)} placeholder="0"
              ...
              <span style={{ fontSize: 12, color: "var(--text3)", fontVariantNumeric: "tabular-nums" }}>KES {amount.toLocaleString()}</span>
            )}
          </div>
        );
      })}
...
function AddStudentModal({ onClose, onAdd }) {
  ...
  const handleToggleFeeType = id => setForm(f => ({
    ...f, selectedFeeTypes: f.selectedFeeTypes.includes(id)
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const res = await axios.post(`${API}/api/students`, {
        ...
        phone: form.phone.trim(), fee: totalDue, paid: paidNum, feeBreakdown, status,
      }, { headers: { Authorization: `Bearer ${token}` } });
      onAdd(res.data);
      onClose();
    } catch (e) {
              ...
              {form.cls && (
                <>
                  <div>
                    <label className="settings-label" style={{ marginBottom: 8, display: "block" }}>
                      ...
                      <span style={{ fontSize: 11, color: "var(--text3)", fontWeight: 400, marginLeft: 6, textTransform: "none" }}>
                        auto-filled from fee structure
                      </span>
                    </label>
                  ...
                  {form.selectedFeeTypes.length > 0 && (
                    <div style={{ background: "rgba(34,211,164,0.04)", border: "1px solid rgba(34,211,164,0.15)", borderRadius: 10, padding: "12px 16px", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                      <div>
...
export default function Students() {
  ...
  const [profileId, setProfileId]     = useState(null); // student id for profile modal
  ...
  const allClasses = useMemo(() => {
    const merged = [...new Set([...classes, ...students.map(s => s.cls)])].filter(Boolean).sort();
    return merged;
  }, [students, classes]);
      ...
      <div className="page-content">
        ...
        <div style={{ display: "flex", gap: 10, marginBottom: 20, flexWrap: "wrap" }}>
          {[
            ...
            <div key={s.key} onClick={() => setFilterStatus(s.key)} style={{
              ...
              borderRadius: 10, padding: "10px 14px", cursor: "pointer", minWidth: 80, flex: "1 1 80px", maxWidth: 140,
            }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 16, flexWrap: "wrap" }}>
          <div style={{ position: "relative", flex: "1 1 200px", minWidth: 160 }}>
            ...
            <input style={{ width: "100%", paddingLeft: 36, paddingRight: 12, height: 40, background: "var(--surface)", border: "1px solid var(--border)", borderRadius: 9, color: "var(--text)", fontSize: 14, outline: "none", boxSizing: "border-box", fontFamily: "inherit" }}
              placeholder="Search by name, class or adm…" value={search} onChange={e => setSearch(e.target.value)} />
          </div>
        ...
        <div style={{ background: "var(--surface)", border: "1px solid var(--border)", borderRadius: 12, overflow: "hidden" }}>
          {loading ? (
            ...
            <div style={{ padding: 40, textAlign: "center", color: "var(--red)", fontSize: 13 }}>{loadErr}</div>
          ) : filtered.length === 0 ? (
            ...
            filtered.map(s => <StudentCard key={s.id} s={s} onClick={() => setProfileId(s.id)} />)
          ) : (
                  ...
                  {filtered.map(s => {
                    ...
                    return (
                      <tr key={s.id} onClick={() => setProfileId(s.id)} style={{ cursor: "pointer" }}>
                        ...
                        <td style={{ color: balance === 0 ? "var(--text3)" : "var(--amber)", fontVariantNumeric: "tabular-nums", fontWeight: 500 }}>
