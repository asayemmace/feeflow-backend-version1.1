...
export default function AddStudentModal({ onClose, onSave }) {
  const [formData, setFormData] = useState({
    name: '', adm: '', className: 'Form 1A', fee: '', status: 'Unpaid', paid: ''
  });

  const handleSubmit = async (e) => {
    e.preventDefault();

    const chargeAmount = Number(formData.fee);
    if (!formData.name || !formData.className || Number.isNaN(chargeAmount)) return;
    if (!formData.adm.trim()) {
    ...
    if (paid > chargeAmount) paid = chargeAmount;
    if (formData.paid === '') {
      paid = formData.status === 'Paid' ? chargeAmount : 0;
    }

    try {
      await onSave({
        name: formData.name,
        adm: formData.adm.trim(),
